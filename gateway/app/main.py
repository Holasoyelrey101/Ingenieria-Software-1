    ...
    return [
        {
            ...
            "status": i.status,
            "created_at": (lambda dt: dt.isoformat() if dt else None)(getattr(i, "created_at", None)),
        }
        for i in items
    ]
...
def get_route_request(request_id: int, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "status": item.status,
        "created_at": (lambda dt: dt.isoformat() if dt else None)(getattr(item, "created_at", None)),
    }

# ------------------------------------------------------
# IMPORTACIÓN DE RUTAS ADICIONALES (HU11, HU12, HU6)
# ------------------------------------------------------
...
# 🔹 Nuevo router: HU6 - Camiones (Visualización de cámaras)
try:
    from app.routers import camiones_router
    app.include_router(camiones_router.router)
    logging.info("✅ Módulo de camiones (HU6) cargado correctamente.")
except ImportError as e:
    logging.warning(f"No se pudo importar el módulo de camiones: {e}")
