...
export default function MapView() {
  ...
  // Estado para preview automático de ruta
  const [routePreview, setRoutePreview] = useState<{ coords: google.maps.LatLngLiteral[]; distance_m: number; duration_s: number } | null>(null)
  ...
  // Preview automático de ruta cuando se seleccionan origen y destino
  React.useEffect(() => {
  ...
  // Función para obtener preview de ruta
  const fetchRoutePreview = async () => {
    if (!originPlace?.location || !destPlace?.location) return

    try {
      setLoadingPreview(true)
      console.log('🔍 Obteniendo preview de ruta...')
      ...
      if (res.data?.polyline) {
        const coords = decodePolyline(res.data.polyline)
        setRoutePreview({
          coords,
          distance_m: res.data.distance_m || 0,
          duration_s: res.data.duration_s || 0
        })
        console.log(`✓ Preview: ${(res.data.distance_m / 1000).toFixed(2)}km, ${Math.round(res.data.duration_s / 60)}min`)
      }
    } catch (err) {
      console.warn('⚠️ No se pudo obtener preview de ruta:', err)
      setRoutePreview(null)
    } finally {
        ...
        <div>
          ...
          {routePreview && !route && (
            <div style={{
              ...
              <div style={{ fontWeight: 'bold', marginBottom: '6px' }}>🔍 Preview de Ruta</div>
              ...
              <div style={{ fontSize: '10px', marginTop: '6px', fontStyle: 'italic', color: '#3B82F6' }}>
                {loadingPreview ? 'Actualizando...' : 'Haz click en "Calcular Ruta" para confirmar'}
              </div>
            </div>
          )}
