    ...
    if (Hls.isSupported()) {
      ...
      hls.on(Hls.Events.MANIFEST_PARSED, () => {
        ...
        if (playPromise !== undefined) {
          playPromise.catch((e) => {
            console.warn("[" + camId + "] Autoplay blocked, intentando sin sonido:", e);
            // Si falla, asegurar que esté muted y reintentar
            video.muted = true;
            video.play().catch((e2) => {
              console.error("[" + camId + "] No se pudo reproducir:", e2);
              setError(null); // No mostrar error, solo dejar que el usuario haga clic
            });
          });
        }
      });
